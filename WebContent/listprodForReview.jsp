...
<body>
	...
	if (session.getAttribute("reviewProdMessage") != null)
		{
			out.println("<h1 style=\"color:#FF0000\">"+session.getAttribute("reviewProdMessage").toString()+"</h1>");
		session.removeAttribute("reviewProdMessage");
	}
	
	if (session.getAttribute("currectProductId") != null)
		session.removeAttribute("currectProductId");
	
	
	%>

<h1>Search for the products you want to review:</h1>

<form method="get" action="listprodForReview.jsp">
	    ...
	    {		
			...
			{	
					...
					{	
						...
						String link = "productReview.jsp?id="+rst.getString(1); 
						link =link.replace(" ","+");
						
						//link.replace("'","%27");
						out.print("<td><a href="+link+">Review</a></td>");
