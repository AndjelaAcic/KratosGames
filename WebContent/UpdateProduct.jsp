...
<%
	...
	if(authenticatedUser != null)
		response.sendRedirect("test1.jsp");		// Successful login
	else
		response.sendRedirect("test1.jsp");		// Failed login - redirect back to login page with a message 
%>
	...
	String validateRegister(JspWriter out,HttpServletRequest request, HttpSession session) throws IOException
	{
		...
		try{
					...
					Statement stmt = con.createStatement();) 
				{
					...
					//adding
					String sqlUpdate = "insert into product(productName,productPrice,productImageURL,productDesc,categoryId) values (?,?,?,?,?)";
					PreparedStatement stmtUpdate = con.prepareStatement(sqlUpdate);
					
                    stmtUpdate.setString(1,prodname);
					
					stmtUpdate.setString(3,prodImagURL);
					stmtUpdate.setDouble(2,Double.parseDouble(price));
